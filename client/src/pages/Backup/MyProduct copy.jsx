...
function MyProduct({ productId, image, title, description, cartProductQuantity, productQuantity, age, price, rating, reviewsCount, category, sellerName, sellerId, reviews }) {
        ...
        if (cartItems.length === 0) {
            try {
                ...
                if (response.status === 201) {
                    ...
                    productsDispatch({ type: "updateProductQuantity", payload: { productId, productQuantity } })
                }
            } catch (error) {
                console.log(error)
            }
    ...
    return (
        <Card className='w-100 d-flex flex-md-row border-0 flex-wrap align-items-center align-items-lg-start justify-content-center' >
            <Header className='flex-lg-one-third align-self-center bg-transparent border-0 w-min-sm-75 w-min-md-50 w-min-lg-25'>
                <Img className='object-fit-contain bg-light' src={`${BASE_URL}/uploads/${sellerId}/${image}`} alt='images/PurrStore.svg' />
            </Header>
